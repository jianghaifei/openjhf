import { isChromium, isLinux } from '@/Components/Designer/Layout/utils'
 ...
 */
export class DefaultEventHandlers<O = Record<string, never>> extends CoreEventHandlers<
...
> {
  ...
  draggedElementShadow: HTMLElement | undefined
      ...
      drag: (el: HTMLElement, id: NodeId) => {
        ...
        const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
          ...
          this.dropElement((dragTarget, indicator) => {
            ...
            const index =
              indicator.placement.index + (indicator.placement.where === 'after' ? 1 : 0)
        ...
        const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
          ...
          this.dropElement((dragTarget, indicator) => {
            ...
            store?.actions.history
              .throttle(100)
              .addNodeTree(dragTarget.tree, indicator.placement.parent.id, index)

            if (options && isFunction(options.onCreate)) {
              options.onCreate(dragTarget.tree, e)
            }
          })
        })
  ...
  private dropElement(onDropNode: (dragTarget: DragTarget, placement: Indicator) => void) {
    const store = this.options?.store
    ...
    if (draggedElementShadow) {
      ...
      this.draggedElementShadow = undefined
    }
