...
const Methods = (
  ...
  return {
    ...
    add(nodeToAdd: Node | Node[], parentId: NodeId, index?: number) {
      ...
      nodes.forEach((node: Node) => {
        addNodeTreeToParent(
          ...
          parentId,
          { type: 'child', index: index as number }
        );
      });
    },
    ...
    addNodeTree(tree: NodeTree, parentId?: NodeId, index?: number) {
      addNodeTreeToParent(tree, parentId, { type: 'child', index: index as number });
    },
    ...
    move(selector: NodeSelector, newParentId: NodeId, index: number) {
      ...
      targets.forEach(({ node: targetNode }, i) => {
        ...
        if (!query.node(targetNode.id).isDraggable()) {
          return false
        }
        ...
        state.options.onBeforeMoveEnd(
          targetNode,
          newParent,
          state.nodes[currentParentId as string]
        );

        const currentParent = state.nodes[currentParentId as string];
