...
export function NodeHelpers(state: EditorState, id: NodeId) {
  ...
  return {
    ...
    ancestors(deep = false): NodeId[] {
      ...
      return appendParentNode(node.data.parent!, [], 0);
    },
    descendants(
      deep = false,
    ...
    isDroppable(selector: NodeSelector, onError?: (err: string) => void) {
      ...
      try {
        ...
        const parentNodes: Record<string, any> = {};
        ...
        targets.forEach(({ node: targetNode, exists }) => {
          ...
          const currentParentNode: Node | null =
            targetNode.data.parent ? state.nodes[targetNode.data.parent] : null;
          ...
          if (currentParentNode && currentParentNode.id !== newParentNode.id) {
