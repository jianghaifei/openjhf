...
export const adjustedBoundingRect = (el: HTMLElement) => {
...
const findClosestSnap = (n: number, snapArray: number[], snapGap = 0): number => {
...
export class Resizable extends React.PureComponent<ResizableProps, State> {
  ...
  bindEvents() {
    if (this.window) {
      ...
      this.window.addEventListener('mouseleave', this.onMouseUp.bind(this))
      this.window.addEventListener('touchmove', this.onMouseMove.bind(this), {
        capture: true,
        passive: false
      })
      this.window.addEventListener('touchend', this.onMouseUp.bind(this))
    }
  }

  unbindEvents() {
    if (this.window) {
      this.window.removeEventListener('mouseup', this.onMouseUp.bind(this))
      this.window.removeEventListener('mousemove', this.onMouseMove.bind(this))
      this.window.removeEventListener('mouseleave', this.onMouseUp.bind(this))
      this.window.removeEventListener('touchmove', this.onMouseMove.bind(this), true)
      this.window.removeEventListener('touchend', this.onMouseUp.bind(this))
    }
  }
  ...
  setBoundingClientRect() {
    ...
    if (this.props.bounds === 'parent') {
      const parent = this.parentNode
      if (parent) {
        const parentRect = adjustedBoundingRect(parent) // .getBoundingClientRect()
    ...
    if (this.props.bounds && typeof this.props.bounds !== 'string') {
      const targetRect = adjustedBoundingRect(this.props.bounds) // .getBoundingClientRect()
    ...
    // For boundary
    if (this.resizable) {
      const { left, top, right, bottom } = adjustedBoundingRect(this.resizable) // .getBoundingClientRect()
  ...
  onMouseUp(event: MouseEvent | TouchEvent) {
    ...
    console.log('Resizable: delta: ', delta)
    if (this.props.onResizeStop) {
      this.props.onResizeStop(event, direction, this.resizable, delta)
      console.log('Resizable: onResizeStop: ', direction, this.resizable, delta)
    }
    if (this.props.size) {
      this.setState(this.props.size)
      console.log('Resizable: this.setState(this.props.size): ', this.props.size)
    }
    this.unbindEvents()
    this.setState({
      isResizing: false,
