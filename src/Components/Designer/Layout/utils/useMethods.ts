...
export function useMethods<S, R extends MethodRecordBase<S>, Q extends QueryMethods>(
  ...
  const ignoreHistoryForActionsRef = useRef<string[]>([])
  ...
  const reducer = useMemo(() => {
    ...
    return (state: S, action: Action) => {
      ...
      let finalState
      let [nextState, patches, inversePatches] = produceWithPatches(state, (draft: S) => {
        switch (action.type) {
  ...
  const actions = useMemo(() => {
    const actionTypes = Object.keys(methodsFactory(null as any, null))
...
class Subscriber {
   ...
   */
  constructor(collector: any, onChange: any, collectOnCreate = false) {
