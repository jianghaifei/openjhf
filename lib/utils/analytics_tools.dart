...
class AnalyticsTools {
  ...
  /// 对比视图
  static Widget buildCompareWidget(ModuleMetricsCardCompValue? compValueEntity, {bool? enableTitle=true}) {
    List<Widget> widgets = [];
    if (compValueEntity?.dayCompare != null) {
      widgets.add(AnalyticsTools()._buildCompareSubView(CompareDateRangeType.yesterday,
          compValueEntity?.dayCompare?.displayValue, compValueEntity?.dayCompare?.color, enableTitle:enableTitle));
    }
    if (compValueEntity?.weekCompare != null) {
      widgets.add(AnalyticsTools()._buildCompareSubView(CompareDateRangeType.lastWeek,
          compValueEntity?.weekCompare?.displayValue, compValueEntity?.weekCompare?.color, enableTitle:enableTitle));
    }
    if (compValueEntity?.monthCompare != null) {
      widgets.add(AnalyticsTools()._buildCompareSubView(CompareDateRangeType.lastMonth,
          compValueEntity?.monthCompare?.displayValue, compValueEntity?.monthCompare?.color, enableTitle:enableTitle));
    }
    if (compValueEntity?.yearCompare != null) {
      widgets.add(AnalyticsTools()._buildCompareSubView(CompareDateRangeType.lastYear,
          compValueEntity?.yearCompare?.displayValue, compValueEntity?.yearCompare?.color, enableTitle:enableTitle));
    }
  ...
  Widget _buildCompareSubView(CompareDateRangeType type, String? displayValue, String? colorString, {bool? enableTitle=true}) {
    ...
    // zlj 添加，解决ai chat不显示对比数据标题
    if(enableTitle == false){
      title = '';
    }
    return Row(
