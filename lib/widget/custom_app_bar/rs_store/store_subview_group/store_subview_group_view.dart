...
class _StoreSubviewGroupPageState extends State<StoreSubviewGroupPage> with AutomaticKeepAliveClientMixin {
              ...
              children: [
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: _createStoreNameAndIdWidget(
                        index == -1
                            ? "${S.current.rs_location_select_all}(${shops?.where((e) => e.isEditable).toList().length ?? 0})"
                            : RSAccountManager().findShopName(shops?[index].shopId),
                        index == -1 ? null : shops?[index].shopId,
                        editableValue),
                  ),
                ),
              ],
  ...
  Widget _createStoreNameAndIdWidget(String storeName, String? storeId, bool editableValue) {
    return Column(
        ...
        if (storeId != null)
          Padding(
            ...
            child: Text(
              'ID:$storeId',
              style: TextStyle(
                color: RSColor.color_0x40000000.withOpacity(editableValue ? 0.4 : 0.2),
                fontSize: 10,
                fontWeight: FontWeight.w400,
              ),
            ),
