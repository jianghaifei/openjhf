...
class AnalyticsGroupSalesCardLogic extends GetxController {
  ...
  Future<void> getMetricsData(
    ...
    await request(() async {
      ...
      if (dateType == GroupDataStyle.PIE) {
        size = int.parse(state.metadata.value.chartType?.first.pageSize ?? '${state.showMaxLength - 1}');
      } else {
      ...
      // filter
      if (state.metadata.value.filterInfo != null &&
          state.metadata.value.filterInfo?.filters != null &&
          state.metadata.value.filterInfo!.filters!.isNotEmpty) {
        var filters = state.metadata.value.filterInfo?.filters?.map((element) => element.toJson()).toList();
  ...
  void setChartData() {
    ...
    if (state.resultModuleGroupMetricsEntity.value.reportData == null ||
      ...
      listFlSpot.add(RSChartData("one", "one", 100, RSColor.listColorOther));
    } else {
      int index = 0;
      state.resultModuleGroupMetricsEntity.value.reportData?.rows?.forEach((element) {
        ...
        listFlSpot.add(RSChartData(tmpMetric?.code ?? "$index", tmpMetric?.code ?? "$index",
            tmpMetric?.proportion ?? 0.0, RSColor.getChartColor(index)));
  ...
  void getTestData(
    ...
    listFlSpot.addAll([RSChartData("one", "one", 100, RSColor.listColorOther)]);
