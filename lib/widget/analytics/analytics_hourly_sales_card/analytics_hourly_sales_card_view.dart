...
class _AnalyticsHourlySalesCardPageState extends State<AnalyticsHourlySalesCardPage>
  ...
  String getSeriesName(String name) {
    return widget.from == 'ai_chat' ? S.current.rs_reference : name;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      return MetricCardGeneralWidget(
        ...
        padding: widget.padding ?? EdgeInsets.only(top: 16, bottom: widget.pageEditing ? 16 : 8),
    ...
    if (!logic.getIsShowCompareLine()) {
      ...
      return [
        ...
        if (state.allChartDayCompData.isNotEmpty)
          ColumnSeries<RSChartData, String>(
            name: getSeriesName(S.current.rs_date_tool_yesterday),
        ...
        if (state.allChartWeekCompData.isNotEmpty)
          ColumnSeries<RSChartData, String>(
            name: getSeriesName(S.current.rs_date_tool_last_week),
        ...
        if (state.allChartMonthCompData.isNotEmpty)
          ColumnSeries<RSChartData, String>(
            name: getSeriesName(S.current.rs_date_tool_last_month),
        ...
        if (state.allChartYearCompData.isNotEmpty)
          ColumnSeries<RSChartData, String>(
            name: getSeriesName(S.current.rs_date_tool_last_year),
    ...
    return Column(
      ...
      children: [
        ...
        if (compValueEntity != null)
          widget.pageEditing
              ? const Text('-')
              : AnalyticsTools.buildCompareWidget(compValueEntity, enableTitle: widget.from != 'ai_chat'),
      ],
    );
  }
