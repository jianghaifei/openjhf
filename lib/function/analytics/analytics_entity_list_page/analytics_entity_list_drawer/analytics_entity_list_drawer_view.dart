...
class _AnalyticsEntityListDrawerPageState extends State<AnalyticsEntityListDrawerPage> {
  ...
  List<AnalyticsEntityFilterComponentFilters>? myFilters;
  ...
  void initState() {
    ...
    // 深拷贝 filters
    myFilters = widget.filters?.map((e) => AnalyticsEntityFilterComponentFilters.fromJson(e.toJson())).toList();
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Offstage(
              offstage: !logic.getIfShowRange(myFilters),
              child: Obx(() {
  ...
  void clickAction(String action) {
    if (action == "cleanAllAction") {
      setState(() {
        logic.cleanAllData(myFilters);
      });
    } else if (action == "applyAction") {
      var limitList = logic.returnLimitList();
      logic.handleModelData(limitList, myFilters);

      widget.applyCallback.call(limitList, state.currentSymbol.value, myFilters);
